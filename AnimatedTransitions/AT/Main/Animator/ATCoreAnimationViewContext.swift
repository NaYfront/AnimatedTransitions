...
extension CALayer {
  internal static var atAddedAnimations: [(CALayer, String, CAAnimation)]? = {
    ...
    let swizzledSelector = #selector(at_add(anim:forKey:))
    swizzling(CALayer.self, originalSelector, swizzledSelector)
    return nil
  }()

  @objc dynamic func at_add(anim: CAAnimation, forKey: String?) {
    if let animationKey = forKey,
        CALayer.atAddedAnimations != nil,
        let copiedAnim = anim.copy() as? CAAnimation {
        copiedAnim.delegate = nil // having delegate resulted some weird animation behavior
        CALayer.atAddedAnimations?.append((self, animationKey, copiedAnim))
    }
    at_add(anim: anim, forKey: forKey)
  }
}

internal class ATCoreAnimationViewContext: ATAnimatorViewContext {
  ...
  func uiViewBasedAnimate(duration: TimeInterval, delay: TimeInterval, _ animations: @escaping () -> Void) {
    ...
    if let (stiffness, damping) = targetState.spring {
      ...
      let addedAnimations = CALayer.atAddedAnimations!
      CALayer.atAddedAnimations = nil
  ...
  func addAnimation(_ animation: CAAnimation, for key: String, to layer: CALayer) {
    let atAnimationKey = "at.\(key)"
    animations.append((layer, atAnimationKey, animation))
    layer.add(animation, forKey: atAnimationKey)
  }
   ...
   */
  func viewState(targetState: ATTargetState) -> [String: Any?] {
  ...
  override func apply(state: ATTargetState) {
  ...
  override func changeTarget(state: ATTargetState, isDestination: Bool) {
    let targetState = viewState(targetState: state)
    for (key, targetValue) in targetState {
      let from: Any?, to: Any?
  ...
  override func startAnimations() -> TimeInterval {
    if let beginStateModifiers = targetState.beginState {
      let beginState = ATTargetState(modifiers: beginStateModifiers)
